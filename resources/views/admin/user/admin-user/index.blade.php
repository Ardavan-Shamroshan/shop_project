...
@section('content')
    ...
    <section class="main-body-container">
        ...
        <section class="body-content d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
            <a href="{{ route('admin.user.admin-user.create') }}" class="btn btn-info  btn-hover color-8 rounded-lg">ایجاد ادمین جدید</a>
...
{{--                        <td>{{ $admin->national_code ?? '-' }}</td>--}}
                        ...
                        <td class="width-16-rem text-left">
                            <a href="{{ route('admin.user.admin-user.roles', $admin) }}" class="btn btn-warning  btn-hover rounded-lg border color-4"><i class="fa fa-user-cog"></i> نقش</a>
                            <a href="{{ route('admin.user.admin-user.permissions', $admin) }}" class="btn btn-info  btn-hover rounded-lg border color-5"><i class="fa fa-user-shield"></i> سطوح دسترسی</a>
                            <a href="{{ route('admin.user.admin-user.edit', $admin->id) }}" class="btn btn-primary  btn-hover border rounded-lg  color-9"><i class="fa fa-pen font-size-12"></i> ویرایش</a>
                            <form action="{{ route('admin.user.admin-user.destroy', $admin->id) }}" class="d-inline" method="post">
                                @csrf
                                @method('delete')
                                <button type="submit" class="btn btn-danger  btn-hover color-11 delete border rounded-lg ">

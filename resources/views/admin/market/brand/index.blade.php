...
@section('content')
    ...
    <section class="main-body-container">
        ...
        <section class="body-content d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
            <a href="{{ route('admin.market.brand.create') }}" class="btn btn-info btn-sm border rounded-lg btn-sm btn-hover color-8">ایجاد برند جدید</a>
                ...
                @forelse($brands as $brand)
                    <tr>
                        ...
                        <td class="shadow-sm text-center">
                            <img src="{{ asset($brand->logo['indexArray'][$brand->logo['currentImage']]) }}" alt="عکس" width="70" height="50" class="border rounded">
                        </td>
                        <td>{{ $brand->slug }}</td>
                        <td>{{ $brand->tags }}</td>
                        <td>
                            <label for="">
                                <input type="checkbox" id="{{ $brand->id }}"
                                       ...
                                       data-value="{{ $brand->status }}"
                                       @if($brand->status === 1) checked @endif>
                            </label>
                        </td>
                        <td class="width-16-rem text-left">
                            <a href="{{ route('admin.market.brand.edit', $brand->id) }}"
                               class="btn btn-primary btn-sm border rounded-lg btn-sm btn-hover color-9"><i class="fa fa-pen font-size-12"></i> ویرایش </a>

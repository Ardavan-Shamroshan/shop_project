    ...
    <section class="main-body-container">
        ...
        <section class="body-content d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
            <a href="{{ route('admin.market.category.create') }}"
               class="btn btn-info btn-sm border rounded-pill btn-hover color-8">ایجاد دسته بندی</a>
            ...
            <table class="table table-striped table-hover">
                ...
                @foreach($productCategories as $productCategory)
                    <tr>
                        ...
                        <td>
                            <label>
                                <input type="checkbox" id="{{ $productCategory->id }}-showInMenu" onchange="showInMenu({{ $productCategory->id }})"
                                       data-url="{{ route('admin.market.category.showInMenu', $productCategory->id) }}"
                                       @if ($productCategory->show_in_menu === 1)checked @endif>
                            </label>
                        </td>
                        <td>
                            <label for="">
                                <input type="checkbox" id="{{ $productCategory->id }}"
                                       onchange="changeStatus({{ $productCategory->id }})"
                                       data-url="{{ route('admin.market.category.status', $productCategory->id) }}"
                                       data-value="{{ $productCategory->status }}"
                        ...
                        <td class="width-16-rem text-left">
                            <a href="{{ route('admin.market.category.edit', $productCategory->id) }}"
                               class="btn btn-primary btn-sm border rounded-pill btn-hover color-9"><i
